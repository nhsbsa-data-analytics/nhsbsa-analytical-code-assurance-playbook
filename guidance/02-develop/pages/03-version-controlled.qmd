...
Pull requests are not limited to just merges into `main` but are the only pull requests that are mandatory.
...
Semantic versioning is a standard that is used across software engineering to denote meaningful version numbers. You will have likely seen them already when downloading software such as R (R-4.5.3 is latest version as of writing).
...
Semantic versioning can also be applied to analytical outputs or outputs of analytical code. A simple approach to this is to combined the code and data versions to make a traceable ID. For example:
